...
The extension operates in FOUR main execution contexts:
...
- Plugins are loaded with dependency management via
  ...
  - [`cs-loader-registry.ts`](../src/utils/cs-loader-registry.ts), [`loaders.ts`](../src/entrypoints/content-scripts/loaders/loaders.ts): for non-UI plugins
...
- Persistence via `extension-local-storage` and `indexed-db` services
