...
export default function ForceWritingModeToggle() {
    ...
    >
      <Tooltip content={"Writing mode"}>
        <DialogTrigger asChild>
          <button
            className={cn(
              "x:-order-2 x:flex x:size-8 x:cursor-pointer x:items-center x:justify-center x:gap-1 x:rounded-full x:text-center x:text-sm x:font-medium x:text-muted-foreground x:transition-all x:duration-150 x:outline-none x:placeholder:text-muted-foreground x:hover:bg-primary-foreground x:focus-visible:bg-primary-foreground x:focus-visible:outline-none x:active:scale-95 x:disabled:cursor-not-allowed x:disabled:opacity-50",
              {
